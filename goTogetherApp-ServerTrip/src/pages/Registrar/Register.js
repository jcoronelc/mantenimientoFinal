...
import { registerUser, getProfileById } from "../../services/profile";
...
function Register() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const newProfileId = registeredUser.id;
            console.log(registeredUser.id)
            const newProfile = await getProfileById(newProfileId);
            console.log('Nuevo perfil registrado:', newProfile);
            setRedirect(true);
        } catch (error) {
                ...
                <div className="flex justify-center items-center">
                    <div className="w-full max-w-md p-6 mb-4 ">
                        ...
                        <form className="space-y-4 pt-2" onSubmit={handleSubmit}>
                            ...
                            <button type="submit" className="w-full text-blue-50 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm py-3">Registrarse</button>
                            <div className="flex flex-row items-center justify-center">
                                <div className="flex mr-2">
                                    <p className="text-sm text-gray-600">¿Ya tienes una cuenta?</p>
                                </div>
                                <div className="flex">
                                    <label onClick={handleRedirect} className="text-blue-700 text-sm font-bold">Iniciar sesión</label>
                                </div>
                            </div>
                        </form>
                    </div>
