...
function VehicleCard({ name, brand, color, passengers, plate, onDelete, index,  vehicle_id }) {
    return (
        <div className="flex bg-white border border-gray-200 rounded-lg shadow-md mt-2 mb-2 w-full">
            <div className="flex flex-col m-4 space-y-4 w-full">
                <div className="flex flex-row justify-between items-center m-2">
                    ...
                    <div className="flex">
                    <button type="button"
                            className="w-10 h-10  text-blue-50 bg-red-50 hover:bg-red-600 focus:ring-2 font-medium rounded-lg text-sm  text-center" title='Eliminar' onClick={() => onDelete(index, vehicle_id)}>
